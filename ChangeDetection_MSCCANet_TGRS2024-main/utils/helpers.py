...
from models.MSCCANet import MSTransception
import os
...
# 传入的net_name = "MSCCANet"，实际是MSCCANet.py中的class MSTransception(nn.Module)
def load_model(net_name,opt, device):
    """Load the model
    ...
    if net_name == "SNUNet_ECAM":
        model = SNUNet_ECAM(opt.num_channel, 2).to(device)
    elif net_name == "UTNetV2":
        model = UTNetV2(opt.num_channel, 2).to(device)
    elif net_name == "MSTransception":    
        model = MSTransception(num_classes=2).to(device)
    elif net_name == "MSCCANet":
        model = MSTransception(num_classes=2).to(device)
        
        MSTransception
        # os.environ['CUDA_VISIBLE_DEVICES'] = "0,1,2,3,4,5,6,7" #设置所有可以使用的显卡，共计四块
        # device_ids = [6,7]#选中其中两块
        # model = UTNetV2(opt.num_channel, 2)
