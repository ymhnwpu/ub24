...
# 多尺度特征提取模块。P8 Fig.4.的Stage0~Stage4。Patch Embedding-》多头注意力模块-》多尺度特征提取
...
class MSTransception(nn.Module):
    # MSTransception模型，集成了多尺度特征提取和桥接模块
    # num_classes: 分类数目
    # head_count: 注意力头数
    # dil_conv: 是否使用膨胀卷积
    # token_mlp_mode: Token MLP模式
    # MSViT_config: MSViT配置
    # concat: 特征拼接方式
    # have_bridge: 是否使用桥接模块
    # use_sa_config: 使用的空间注意力配置
    # sa_ker: 空间注意力核大小
    # Stage_3or4: 使用的阶段数（3或4）
    # inter: 中间层处理方式
    # num_sp: 使用的空间注意力层数
    # br_ch_att_list: 桥接模块的通道注意力列表
    # inception: Inception模块配置（可选）
    def __init__(self, num_classes=9, head_count=8, dil_conv=1, token_mlp_mode="mix_skip", MSViT_config=2, concat='coord', have_bridge='original', use_sa_config = 1,
        ...
        if concat != "cbam" or Stage_3or4 == 4:
            use_sa_list = [True, True, True, False]
        # 主干网络选择。使用MSViT进行多尺度特征提取【论文P8 Fig.4.可知使用4阶段，下面桥接处理也是4层】
        if Stage_3or4 == 4:
            ...
            self.backbone = MSViT_casa(image_size=256, in_dim=dims, key_dim=key_dim, value_dim=value_dim, layers=layers,
                            head_count=head_count, dil_conv=dil_conv, token_mlp=token_mlp_mode, MSViT_config=MSViT_config, concat=concat, use_sa_list=use_sa_list, sa_ker=sa_ker, inter=inter)    
        ...
        # Bridge
        # 桥接模块配置。桥接模块使用双向注意力机制，使用BridgeBlock加强特征交互
...
if __name__ == "__main__":
    ...
    x1 = torch.rand(1, 3, 256, 256).cuda()  # 时间点1的输入（变化前图像）
    x2 = torch.rand(1, 3, 256, 256).cuda()  # 时间点2的输入（变化后图像）
    tmp_0 = model(x1, x2)
    # print(tmp_0.shape)
    print(tmp_0[0].shape)
    # summary(model, (1, 3,256,256))
    summary(model, 
            input_size=[(1, 3, 256, 256), (1, 3, 256, 256)],  # 注意这里需要两个输入尺寸
            device='cuda',
